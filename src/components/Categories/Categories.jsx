...
const Categories = () => {
    ...
    return (
      <Tabs className="overflow-x-hidden w-full">
        <TabList className="dark:text-white w-full">
        ...
        <TabPanel>
          <div className="grid dark:bg-black ml-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 gap-y-3">
            {webJob.length == 0
              ? allJob
                  .filter((job) => job.selectedCategory == "Web Development")
                  .map((job) => (
                    <div
                      data-aos="fade-right"
                      key={job._id}
                    ...
                    >
                      <div className="card-body dark:text-white  dark:bg-black ">
                        <h2 className="card-title dark:text-white  text-xl font-bold">
                          {" "}
                          {job?.jobTitle}{" "}
                          <span className="badge flex text-white  bg-[#327289] ">
                            Full time
                          </span>
                        </h2>
                        <p className="dark:text-white ">
                          ...
                          : {job?.minimumPice}$ - {job?.maximumPrice}${" "}
                        </p>
                        <p>Deadline: {job?.deadline} </p>
                        <p> {job?.description.slice(0, 200)} </p>
                        <div className="card-actions justify-end">
                          <Link to={`jobs/${job._id}`}>
                            <button className="btn bg-[#327289] text-white font-semibold">
                              Bid Now
                            </button>
                          </Link>
                        </div>
              ...
              : webJob.map((job) => (
                  ...
                  >
                    <div
                      data-aos="fade-right"
                      className="card-body dark:bg-black dark:text-white "
                    >
                      <h2 className="card-title dark:text-white  text-xl font-bold">
                        {" "}
                        {job?.jobTitle}{" "}
              ...
              >
                <div className="card-body dark:bg-black dark:text-white ">
                  ...
                  <p>
                    ...
                    <span className="text-red-500 font-semibold">
                      Price
                    </span>: {job?.minimumPice}$ - {job?.maximumPrice}{" "}
                  </p>
                  <p>Deadline: {job?.deadline} </p>
                  <p> {job?.description.slice(0, 200)} </p>
                  <div className="card-actions justify-end">
